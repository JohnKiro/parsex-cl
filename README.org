...
This is an experimental regex project, with "lispy" syntax, that could hopefully be part of a bigger lexer-parser system. At this stage, I'm fairly satisfied with the regex feature coverage it provides. However, it is not meant to compete with the well-established, feature-rich *[[https://github.com/edicl/cl-ppcre][CL-PPCRE]]*.

* Status
While it's still at an early experimental status, it is already capable of matching strings against classical *regular expressions*, that are provided in the form of *s-expressions*. See section entitled [[unit-tests][Unit Tests]] for information about how to experiment with the regex matcher.
