...
it's called. Note that client code could use `input-source` to retrieve last accumulated text. Also note
that if input is already exhausted, the tokenization function returns NIL."
  ...
  (labels ((get-tokens ()
             ...
             (let ((result (match:match-regex input-source tokenizer-core-dfa)))
               ...
               ;; :regex-matched)
               (and result (match:regex-matching-result-tokens result)))))
    #'get-tokens))
